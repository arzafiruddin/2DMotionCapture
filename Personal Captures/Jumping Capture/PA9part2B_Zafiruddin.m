...
% 06/21/2021
% PA9part2B_Zafiruddin.m
%
% Takes video of gait motion, tracks the positions of the joints, and
% creates wireframe capture. (Part 2B: Ameen's Jump Video)

clear
clc
close all

delete jumpVidCentroidAmeenB.avi
delete jumpVidWireframeAmeenB.avi
...
% VIDEO INFORMATION
vidI = VideoReader('jumpVidAmeen.mov'); % reads in .avi file
frameStart = 100; % starting frame for motion capture
frameStop = 205; % ending frame for motion capture

% VIDEO CROPPING DIMENSIONS
xCropO = 1; % top-left x (col) point of crop
yCropO = 200; % top-left y (row) point of crop
xCropL = 1920; % horizontal length of crop
yCropL = 880; % vertical length of crop
...
vidFA = VideoWriter('jumpVidCentroidAmeenB.avi');
...
for k = frameStart:frameStop
    ...
    subplot(3,1,3) % plots joint centroids dynamically
        ...
        axis([xCropO, xCropO+xCropL, yCropO, yCropO+yCropL]) % fixes x & y 
                                                             % axes' limits
        pbaspect([2.17,1,1]) % sets aspect ratio of plot to that of video
...
vidFB = VideoWriter('jumpVidWireframeAmeenB.avi');
