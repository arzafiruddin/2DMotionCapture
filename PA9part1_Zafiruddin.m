...
% 06/21/2021
% PA9part1_Zafiruddin.m
%
% Takes video of gait motion, tracks the positions of the joints, and
% creates wireframe capture. (Part 1: Naji's Video)

clear
clc
close all

delete walkVidCentroid.avi
delete walkVidWireframe.avi
...
% VIDEO INFORMATION
vidI = VideoReader('walkVid.mov'); % reads in .avi file
frameStart = 123; % starting frame for motion capture
frameStop = 197; % ending frame for motion capture

% VIDEO CROPPING DIMENSIONS
xCropO = 1; % top-left x (col) point of crop
yCropO = 400; % top-left y (row) point of crop
xCropL = 1920; % horizontal length of crop
yCropL = 700; % vertical length of crop

% RGB THRESHOLDING VALUES [RED CENTROID]
thUR_R = 255; % red upper threshold (0-255) for Red Centroid
thLR_R = 200; % red lower threshold (0-255) for Red Centroid
thUG_R = 70; % green upper threshold (0-255) for Red Centroid
thLG_R = 0; % green lower threshold (0-255) for Red Centroid
thUB_R = 140; % blue upper threshold (0-255) for Red Centroid
thLB_R = 60; % blue lower threshold (0-255) for Red Centroid

% RGB THRESHOLDING VALUES [GREEN CENTROID]
thUR_G = 120; % red upper threshold (0-255) for Green Centroid
thLR_G = 0; % red lower threshold (0-255) for Green Centroid
thUG_G = 255; % green upper threshold (0-255) for Green Centroid
thLG_G = 150; % green lower threshold (0-255) for Green Centroid
thUB_G = 170; % blue upper threshold (0-255) for Green Centroid
thLB_G = 40; % blue lower threshold (0-255) for Green Centroid

% RGB THRESHOLDING VALUES [BLUE CENTROID]
thUR_B = 100; % red upper threshold (0-255) for Blue Centroid
thLR_B = 0; % red lower threshold (0-255) for Blue Centroid
thUG_B = 255; % green upper threshold (0-255) for Blue Centroid
thLG_B = 150; % green lower threshold (0-255) for Blue Centroid
...
thLR_Y = 190; % red lower threshold (0-255) for Yellow Centroid
thUG_Y = 255; % green upper threshold (0-255) for Yellow Centroid
thLG_Y = 190; % green lower threshold (0-255) for Yellow Centroid
thUB_Y = 80; % blue upper threshold (0-255) for Yellow Centroid
thLB_Y = 0; % blue lower threshold (0-255) for Yellow Centroid

%% Centroid Capture (Thresholding & Recording)

% opens recording and sets framerate for Centroid Capture
vidFA = VideoWriter('walkVidCentroid.avi');
...
for k = frameStart:frameStop
    ...
    subplot(3,1,3) % plots joint centroids dynamically
        ...
        axis([xCropO, xCropO+xCropL, yCropO, yCropO+yCropL]) % fixes x & y 
                                                             % axes' limits
        pbaspect([2.8,1,1]) % sets aspect ratio of plot to that of video
...
vidFB = VideoWriter('walkVidWireframe.avi');
...
for k = frameStart:frameStop
    ...
    subplot(2,1,2) % plots wireframe model dynamically
        plot(cntrCol_R, (vidI.Height - cntrRow_R), 'r.', ... % R joint
                [cntrCol_R, cntrCol_G], vidI.Height - [cntrRow_R, cntrRow_G], 'r-', ... % R/G line
             cntrCol_G, (vidI.Height - cntrRow_G), 'g.', ... % G joint
                [cntrCol_G, cntrCol_B], vidI.Height - [cntrRow_G, cntrRow_B], 'g-', ... % G/B line
